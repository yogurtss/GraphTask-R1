...
from graphtask_r1.schema import BenchmarkExample, TaskProposal
...
class FrozenSolverService:
    ...
    def _execute_tool(self, backend: GraphBackend, name: str, arguments: dict[str, Any]) -> str:
        if name == "graph_search":
            triples = backend.neighbors(
                [str(value) for value in arguments["entity_ids"]],
                direction=str(arguments.get("direction", "both")),
                relation_ids=[str(value) for value in arguments.get("relation_ids", [])] or None,
                limit=min(int(arguments.get("limit", 50)), 100),
            )
            return json.dumps([value.model_dump(mode="json") for value in triples])
        if name == "inspect_entity":
            return backend.entity_info(str(arguments["entity_id"])).model_dump_json()
        raise ValueError(f"opponent requested unsupported tool: {name}")

    async def rollout(self, task: Any, backend: GraphBackend) -> dict[str, float]:
        ...
        for _ in range(self.max_turns):
            ...
            if calls:
                for call in calls:
                    ...
                    result = self._execute_tool(backend, name, self._arguments(call))
                    messages.append(
                        {
                            "role": "tool",
